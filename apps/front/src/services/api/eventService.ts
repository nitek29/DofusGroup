...
export class EventService {
  ...
  public async getEnrichedEventById(eventId: string): Promise<EventEnriched> {
    const response = await this.axios.get<EventEnriched>(`/event/${eventId}/enriched`);
    return response.data;
  }
  ...
  public async updateEvent(eventId: string, eventData: Partial<Event>): Promise<Event> {
    const response = await this.axios.patch<Event>(`/event/${eventId}`, eventData);
    return response.data;
  }
  ...
  // Admin methods
  public async adminUpdateEvent(userId: string, eventId: string, eventData: Partial<Event>): Promise<EventEnriched> {
    const response = await this.axios.patch<EventEnriched>(`/user/${userId}/event/${eventId}`, eventData);
    return response.data;
  }

  public async adminDeleteEvent(userId: string, eventId: string): Promise<void> {
    await this.axios.delete(`/user/${userId}/event/${eventId}`);
  }

  // Character management
  public async addCharactersToEvent(eventId: string, characterIds: string[]): Promise<EventEnriched> {
    const response = await this.axios.post<EventEnriched>(`/event/${eventId}/addCharacters`, {
      characters_id: characterIds
    });
    return response.data;
  }

  public async removeCharactersFromEvent(eventId: string, characterIds: string[]): Promise<EventEnriched> {
    const response = await this.axios.post<EventEnriched>(`/event/${eventId}/removeCharacters`, {
      characters_id: characterIds
    });
    return response.data;
  }
