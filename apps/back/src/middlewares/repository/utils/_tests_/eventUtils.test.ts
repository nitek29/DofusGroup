...
describe("EventUtils", () => {
  ...
  describe("checkCharactersServer", () => {
    it("should return valid character IDs when all characters are from the same server as the event", () => {
      ...
      const result = eventUtils.checkCharactersServer(mockEvent as EventEntity, validCharacters);
    ...
    it("should throw an error when some characters are from different servers", () => {
      expect(() => {
        eventUtils.checkCharactersServer(mockEvent as EventEntity, mockCharacters as CharacterEntity[]);
      }).toThrow("Following characters aren't from the same server: CharacterIlyzaelle");
    });
    ...
    it("should throw an error listing all invalid characters when multiple characters are from different servers", () => {
      ...
      expect(() => {
        eventUtils.checkCharactersServer(mockEvent as EventEntity, mixedCharacters);
      }).toThrow("Following characters aren't from the same server: CharacterIlyzaelle, CharacterEcho");
    });

    it("should return empty array when no characters are provided", () => {
      const result = eventUtils.checkCharactersServer(mockEvent as EventEntity, []);
    ...
    it("should handle undefined server_id gracefully", () => {
      ...
      expect(() => {
        eventUtils.checkCharactersServer(eventWithNoServer, mockCharacters.slice(0, 1) as CharacterEntity[]);
      }).toThrow("Following characters aren't from the same server: CharacterJiva1");
    });
  });
  ...
  describe("checkTeamMaxLength", () => {
    it("should not throw when adding characters within max_players limit", () => {
      ...
      expect(() => {
        eventUtils.checkTeamMaxLength(eventWithCharacters, ["char-1", "char-2"]);
      }).not.toThrow();
    });
    ...
    it("should throw error when adding characters exceeds max_players", () => {
      ...
      expect(() => {
        eventUtils.checkTeamMaxLength(eventWithManyCharacters, ["char-1", "char-2", "char-3"]);
  ...
  describe("exceptCharactersAlreadyInTeam", () => {
    it("should filter out characters already in the team", () => {
      ...
      const result = eventUtils.exceptCharactersAlreadyInTeam(eventWithCharacters, mockCharacters as CharacterEntity[]);
    ...
    it("should return all characters if none are in the team", () => {
      ...
      const result = eventUtils.exceptCharactersAlreadyInTeam(eventWithNoCharacters, mockCharacters as CharacterEntity[]);
  ...
  describe("exceptCharactersNotInTeam", () => {
    it("should return only characters that are in the team", () => {
      ...
      const result = eventUtils.exceptCharactersNotInTeam(eventWithCharacters, mockCharacters as CharacterEntity[]);
    ...
    it("should return empty array if no characters are in the team", () => {
      ...
      const result = eventUtils.exceptCharactersNotInTeam(eventWithNoCharacters, mockCharacters as CharacterEntity[]);
